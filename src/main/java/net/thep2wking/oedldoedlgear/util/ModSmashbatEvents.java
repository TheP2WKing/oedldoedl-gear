...
public class ModSmashbatEvents {
    ...
    public static void onUpdate(LivingEvent.LivingUpdateEvent event) {
        ...
        if (!entity.world.isRemote && (double)entity.fallDistance > 0.25 && (lightningImpact = ItemLightningSmashbat.lightningEvent.get(entity.getEntityId())) != null) {
            ItemLightningSmashbat.lightningEvent.remove(event.getEntity().getEntityId());
			entity.world.addWeatherEffect(new EntityLightningBolt(entity.world, entity.posX, entity.posY, entity.posZ, false));
        }

		if (!entity.world.isRemote && (double)entity.fallDistance > 0.25 && (explosionImpact = ItemLaunchingSmashbat.explosionEvent.get(entity.getEntityId())) != null) {
            ItemLaunchingSmashbat.explosionEvent.remove(event.getEntity().getEntityId());
			entity.world.newExplosion(null, entity.posX, entity.posY, entity.posZ, 2.0f, false, false);
        }
    }
}