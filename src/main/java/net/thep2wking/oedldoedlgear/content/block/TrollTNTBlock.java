...
public class TrollTNTBlock extends TNTBlock
{
	...
	public TrollTNTBlock(SoundType sound, int harvestLevel, float hardness, float resistance, int lightLevel, Properties properties) 
	{
	...
	public void onExplosionDestroy(World worldIn, BlockPos pos, Explosion explosionIn)
	{
		if (!worldIn.isRemote)
		{
			TrollTNTEntity tntentity = new TrollTNTEntity(worldIn, (double)pos.getX() + 0.5D, (double)pos.getY(), (double)pos.getZ() + 0.5D, explosionIn.getExplosivePlacedBy());
	...
	private static void explode(World worldIn, BlockPos pos, @Nullable LivingEntity entityIn) 
	{
		if (!worldIn.isRemote) 
		{
			TrollTNTEntity tntentity = new TrollTNTEntity(worldIn, (double)pos.getX() + 0.5D, (double)pos.getY(), (double)pos.getZ() + 0.5D, entityIn);
			worldIn.addEntity(tntentity);
	...
	{
        final boolean retval = super.removedByPlayer(state, world, pos, player, willHarvest, fluid);
        if(!player.isCreative() && GearConfig.troll_tnt_multiply_on_break.get())
        {
        	world.setBlockState(pos, ModBlocks.TROLL_TNT.get().getDefaultState());
	        world.setBlockState(pos.up(), ModBlocks.TROLL_TNT.get().getDefaultState());
	        world.setBlockState(pos.down(), ModBlocks.TROLL_TNT.get().getDefaultState());
	        world.setBlockState(pos.north(), ModBlocks.TROLL_TNT.get().getDefaultState());
	        world.setBlockState(pos.west(), ModBlocks.TROLL_TNT.get().getDefaultState());
	        world.setBlockState(pos.south(), ModBlocks.TROLL_TNT.get().getDefaultState());
	        world.setBlockState(pos.east(), ModBlocks.TROLL_TNT.get().getDefaultState());
        }
        return retval;
	}
}