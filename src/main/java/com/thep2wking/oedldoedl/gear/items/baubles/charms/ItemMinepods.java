...
import com.thep2wking.oedldoedl.gear.models.ModelMinepods;
...
public class ItemMinepods extends Item implements IHasModel, IBauble, IRenderBauble
{  
	public ItemMinepods(String name) 
	{
    ...
    {
  		if (GearConfig.GENEREL.effects.baublesGiveEffects && GearConfig.BAUBLES.minepods.effectsEnabled) 
  		{
  			if (itemstack.getItemDamage()==0) 
  			{
  				player.addPotionEffect(new PotionEffect(MobEffects.SPEED, 200, GearConfig.BAUBLES.minepods.speedAmplifier - 1, false, false));
  			}
  		}
    }
	...
	public void onPlayerBaubleRender(ItemStack stack, EntityPlayer player, RenderType type, float partialTicks) 
	{
		...
		GlStateManager.translate(0, 26, -1);
		GlStateManager.rotate(-20, 1, 0, 0);
		if (model==null)
			model = new ModelMinepods();
	...
	public void addInformation(ItemStack stack, World world, List<String> tooltip, ITooltipFlag advanced) 
	{
  		if (CoreConfig.GENEREL.tooltips.itemInformation) 
  		{
  			...
  			{
  				TextFormatting.GREEN+I18n.format("item.oedldoedl.minepods.tip"),
  			});
  		}
  		
  		if (CoreConfig.GENEREL.tooltips.potionInformation && GearConfig.GENEREL.effects.baublesGiveEffects && GearConfig.BAUBLES.minepods.effectsEnabled) 
  		{
  			TooltipControlEffectInfo.addTooltipText(tooltip, new String[] 
  			{
  				...
  				TextFormatting.WHITE+""+TextFormatting.ITALIC+I18n.format("item.oedldoedl.minepods.effects.tip")+TextFormatting.RED+""+TextFormatting.ITALIC+" ("+GearConfig.BAUBLES.minepods.speedAmplifier+") ",
  			});
  		}
	}
}